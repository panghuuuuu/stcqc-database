...
<html>
	<head>
		...
		<!-- title and icon link-->
		<title>  <?php 
          if(isset($_GET['Subject'])){echo $_GET['Subject'];}
...
<body>
  ...
  <?php include 'header_and_footer/header_subject.php'; ?>
  ...
  <div class="container">
    <div class="content">
      ...
      <section id="subjectTitle">
        <br>
        <h1>Researches in <?php 
        //if they select a button("Subject") in index.php, echo that "Subject"
      if(isset($_GET['Subject'])){echo $_GET['Subject'];}
      //elseif they enter submit in the header search bar("submitsearch") in headersubject.php, get the hidden input "SubjectSearch"
      elseif(isset($_GET['submitsearch'])){echo $_GET['SubjectSearch'];}?></h1>
        <br>
      </section>
      ...
      <section id="resultlist">
        ...
        //if they select a button("Subject") in index.php
        ...
        <div id="result">
          <div id="textresult">
            <!-- TITLE -->
            <h1><a href="templateindiv2.php?titleID=<?php echo $row["titleID"]; ?>">
        ...
        <?php
        //elseif they enter submit in the header search bar("submitsearch") in headersubject.php
        }} elseif (isset($_GET['submitsearch'])){
      $search = $_GET['Search'];
      $subject = $_GET['SubjectSearch'];;

        $sql = ("SELECT numberID, titleID, authorsID, yearID, subjectID, abstractID, keywordsID  
            FROM researchpapertable 
            WHERE (titleID LIKE '%" . $search . "%' AND subjectID LIKE '%" . $subject . "%')
            OR (authorsID LIKE '%" . $search . "%' AND subjectID LIKE '%" . $subject . "%')
            OR (yearID LIKE '%" . $search . "%' AND subjectID LIKE '%" . $subject . "%')
            OR (subjectID LIKE '%" . $search . "%' AND subjectID LIKE '%" . $subject . "%')
            OR (abstractID LIKE '%" . $search . "%' AND subjectID LIKE '%" . $subject . "%')
            OR (keywordsID LIKE '%" . $search . "%' AND subjectID LIKE '%" . $subject . "%')
      ...
      if ($result-> num_rows>0) {
        ...
        <?php
        //else their keyword is not found in the Subject they selected
          }
        } else {?>
        <section id="subjects">
          <form action="searchsubject2.php">
